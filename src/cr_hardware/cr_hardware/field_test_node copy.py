...
def update_visualization(next_point, sampled_points, fig1, new_folder_path):
...
def plotGP(drift_values, rpe, fig2):
...
class FieldTestNode(Node):
    ...
    def process_data(self, csv_output_directory):
        ...
        if MOISTURESENSOR:
            try:
                ...
                with open(csv_output_directory + '/' + 'moisture_data_trial'+str(TrialNumber)+'_flag'+str(flag)+'.csv', 'a') as file:
                ...
                data = pd.read_csv(csv_output_directory + '/' + 'moisture_data_trial'+str(TrialNumber)+'_flag'+str(flag)+'.csv')
    ...
    def run(self):
        ...
        new_folder_path = create_folder_with_suffix(base_path + "/" + date + "/" + TrialName + str(TrialNumber))
        ...
        for coordinate in x_true[i_train,:]:
            ...
            if count != 0:
                ...
                update_visualization(list(coordinatePoint), rover_path, fig1, new_folder_path)
            ...
            while cont:
                ...
                cont, data_average = self.process_data(new_folder_path)
        ...
        for j in range(sample_iter):
            ...
            try: 
                ...
                fig2 = plt.figure()
                ax1, ax2, ax3, ax4, ax5 = plotGP(drift_values, retroactive_error, fig2)
                fig2.tight_layout()
